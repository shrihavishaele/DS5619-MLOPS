...
def register_model(name, model_path, metrics, registry_dir):
    ...
    with open(os.path.join(version_dir, "model.json"), "w") as f:
...
def generate_model_card(name, version_id, card_fields, registry_dir):
    ...
    for field in REQUIRED_CARD_FIELDS:
        if field not in card_fields:
            raise ValueError(f"Missing required model card field: '{field}'")
        value = card_fields[field]
        if not value or not value.strip():
            raise ValueError(f"Model card field '{field}' is empty or whitespace-only")
        if "TODO" in value:
    ...
    card = {
        "name": name,
        "version_id": version_id,
        **{field: card_fields[field] for field in REQUIRED_CARD_FIELDS},
    ...
    if target_stage == "Production":
        ...
        if not os.path.exists(card_path):
            raise GovernanceError(
                ...
                f"no model card found. Generate a model card first."
            )
        f1 = manifest["metrics"]["f1"]
        if f1 < PRODUCTION_F1_THRESHOLD:
            raise GovernanceError(
                f"Cannot promote {name} {version_id} to Production: "
                f"f1={f1} is below the required threshold of {PRODUCTION_F1_THRESHOLD}."
            )

    if target_stage == "Production":
        models_dir = os.path.join(registry_dir, "models", name)
        for other_vid in os.listdir(models_dir):
            if other_vid == version_id:
                continue
            other_manifest_path = os.path.join(models_dir, other_vid, "manifest.json")
...
def get_production_model(name, registry_dir):
    ...
    for vid in os.listdir(models_dir):
        manifest_path = os.path.join(models_dir, vid, "manifest.json")
