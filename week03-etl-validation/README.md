...
| | |
|---|---|
| Session time | 110 minutes hands-on |
| Submission deadline | End of day, one day after this session |
| Builds on | Week 2's config-driven pattern (this lab's ETL script is config-driven too) |
| Feeds into | Week 4 (you'll version the *cleaned* output this produces) |
...
Implement a small validation suite in the style of Great Expectations / data
contracts — the same idea covered in this week's lecture — and wire it into an
extract → validate → transform → load pipeline that **refuses to silently pass
through bad data**.
...
## Time budget (of your 110 minutes)

- 5 min: run `generate_for_student.py` with your student ID.
- 10 min: read `data/raw_transactions.csv`, look at `build_expectation_suite()` in
  `src/etl.py` to see what's being checked.
- 40 min: implement the four expectation functions.
- 35 min: implement `run_etl`.
- 15 min: run it, sanity-check the quarantine file, write `NOTES.md`.
- 10 min: commit, push.
...
Source: this lab operationalizes the Data Contracts / Great Expectations & Deequ
content from the Week 3 lecture deck.
